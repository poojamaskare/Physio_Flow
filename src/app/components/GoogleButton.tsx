...
export default function GoogleButton({ onError }: { onError: (msg: string) => void }) {
    ...
    const click = async () => {
        setLoading(true)
        try { await signInWithGoogle() } catch (err) { onError((err as Error).message); setLoading(false) }
    }
    return (
        <>
            <div className="relative my-2 text-center text-xs text-muted-foreground before:absolute before:inset-x-0 before:top-1/2 before:border-t">
